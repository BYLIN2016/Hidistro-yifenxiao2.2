...
function GetCategories(parentCategoryId) {
    var categories = null;
    var postUrl = "SelectMyCategory.aspx?isCallback=true&action=getlist&timestamp=";
...
function GotoNext() {
    ...
    if (productId != "" && productId.length > 0)
        form.action = "editmyproduct.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
    else
        form.action = "EditMyProduct.aspx?categoryId=" + selectedCategoryId;
...
function LoadState(categoryId) {
    $.ajax({
        url: String.format("SelectMyCategory.aspx?isCallback=true&action=getinfo&timestamp={0}&categoryId={1}", new Date().getTime(), categoryId),
        type: 'GET', dataType: 'json', timeout: 10000,
        async: false,
        success: function(resultData) {
            if (resultData.Status == "OK") {
