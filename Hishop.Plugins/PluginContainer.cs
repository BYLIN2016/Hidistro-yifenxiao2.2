    ...
    public abstract class PluginContainer
    {
        ...
        protected PluginItem GetPluginItem(string baseName, string fullName)
        {
            ...
            if (node != null)
            {
                ...
                item2.FullName = node.Attributes["identity"].Value;
                item2.DisplayName = node.Attributes["name"].Value;
                item2.Logo = node.Attributes["logo"].Value;
                item2.ShortDescription = node.Attributes["shortDescription"].Value;
                item2.Description = node.Attributes["description"].Value;
                item = item2;
            }
            return item;
        }
