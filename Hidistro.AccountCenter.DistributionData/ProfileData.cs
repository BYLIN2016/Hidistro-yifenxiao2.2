namespace Hidistro.AccountCenter.DistributionData
{
    ...
    public class ProfileData : PersonalSubsiteProvider
    {
        ...
        public override bool AddBalanceDetail(BalanceDetailInfo balanceDetails)
        {
            ...
            string query = "INSERT INTO distro_BalanceDetails(UserId,UserName,DistributorUserId, TradeDate, TradeType, Income, Balance, Remark) VALUES (@UserId, @UserName,@DistributorUserId, @TradeDate, @TradeType, @Income, @Balance, @Remark)";
        ...
        public override bool AddInpourBlance(InpourRequestInfo inpourRequest)
        {
            ...
            DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Underling_InpourRequest_Create");
            ...
            this.database.AddInParameter(storedProcCommand, "DistributorUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO distro_ShippingAddresses(RegionId,UserId,ShipTo,Address,Zipcode,EmailAddress,TelPhone,CellPhone) VALUES(@RegionId,@UserId,@ShipTo,@Address,@Zipcode,@EmailAddress,@TelPhone,@CellPhone); SELECT @@IDENTITY");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO distro_BalanceDrawRequest (UserId,UserName,DistributorUserId,RequestTime, Amount, AccountName, BankName, MerchantCode, Remark) VALUES (@UserId,@UserName, @DistributorUserId,@RequestTime, @Amount, @AccountName, @BankName, @MerchantCode, @Remark)");
        ...
        {
            DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Underling_ShippingAddress_CreateUpdateDelete");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM distro_InpourRequest WHERE InpourId = @InpourId;");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM distro_MemberGrades WHERE CreateUserId = @CreateUserId");
            this.database.AddInParameter(sqlStringCommand, "CreateUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT count(ShippingId) as Count FROM distro_ShippingAddresses WHERE  UserID = @UserID");
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, userId);
            int num = 0;
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        public override void GetStatisticsNum(out int noPayOrderNum, out int noReadMessageNum, out int noReplyLeaveCommentNum)
        {
            ...
            int num3 = 0;
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("SELECT COUNT(*) AS NoPayOrderNum FROM distro_Orders WHERE UserId = {0} AND OrderStatus = {1} AND DistributorUserId = {2};", HiContext.Current.User.UserId, 1, HiContext.Current.SiteSettings.UserId);
            ...
            builder.AppendFormat("SELECT COUNT(*) AS NoReplyLeaveCommentNum FROM distro_ProductConsultations WHERE UserId = {0} AND ReplyUserId is not null AND ViewDate is null AND DistributorUserId={1};", HiContext.Current.User.UserId, HiContext.Current.SiteSettings.UserId);
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DELETE FROM distro_InpourRequest WHERE InpourId = @InpourId");
            this.database.AddInParameter(sqlStringCommand, "InpourId", DbType.String, inpourId);
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("update distro_ProductConsultations set ViewDate=getdate() WHERE UserId = {0} AND ReplyUserId is not null AND ViewDate is null AND DistributorUserId={1};", HiContext.Current.User.UserId, HiContext.Current.SiteSettings.UserId);
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
