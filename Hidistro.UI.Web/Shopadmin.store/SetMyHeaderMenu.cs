namespace Hidistro.UI.Web.Shopadmin.store
{
    ...
    public class SetMyHeaderMenu : DistributorPage
    {
        ...
        private void BindHeaderMenu()
        {
            ...
            this.grdMyHeaderMenu.DataSource = table;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                node.Attributes["CategoryNum"].Value = num.ToString();
        ...
        private void grdHeaderMenu_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "SetYesOrNo")
            {
                ...
                int num2 = (int) this.grdMyHeaderMenu.DataKeys[rowIndex].Value;
                string filename = HttpContext.Current.Request.MapPath(Globals.ApplicationPath + string.Format("/Templates/sites/" + HiContext.Current.User.UserId.ToString() + "/{0}/config/HeaderMenu.xml", this.themName));
                XmlDocument document = new XmlDocument();
                document.Load(filename);
        ...
        private void grdHeaderMenu_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                link.NavigateUrl = string.Format("EditMyHeaderMenu.aspx?Id={0}&ThemName={1}", num, this.themName);
        ...
        {
            foreach (GridViewRow row in this.grdMyHeaderMenu.Rows)
            {
                int id = (int) this.grdMyHeaderMenu.DataKeys[row.RowIndex].Value;
                int result = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.grdMyHeaderMenu.RowDataBound += new GridViewRowEventHandler(this.grdHeaderMenu_RowDataBound);
            this.grdMyHeaderMenu.RowCommand += new GridViewCommandEventHandler(this.grdHeaderMenu_RowCommand);
            this.grdMyHeaderMenu.RowDeleting += new GridViewDeleteEventHandler(this.grdHeaderMenu_RowDeleting);
