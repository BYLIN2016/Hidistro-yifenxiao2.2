namespace Hidistro.UI.Web.Shopadmin
{
    ...
    public class MyGroupBuys : DistributorPage
    {
        ...
        private void btnOrder_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in this.grdGroupBuyList.Rows)
            {
                ...
                if (int.TryParse(box.Text.Trim(), out result))
                {
                    ...
                    SubsitePromoteHelper.SwapGroupBuySequence(groupBuyId, result);
                }
            }
            this.BindGroupBuy();
        }
        ...
        private void grdGroupBuyList_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            else if (SubsitePromoteHelper.DeleteGroupBuy((int) this.grdGroupBuyList.DataKeys[e.RowIndex].Value))
            {
                this.BindGroupBuy();
        ...
        private void lkbtnDeleteCheck_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in this.grdGroupBuyList.Rows)
            {
                ...
                {
                    num2++;
                    SubsitePromoteHelper.DeleteGroupBuy(Convert.ToInt32(this.grdGroupBuyList.DataKeys[row.RowIndex].Value, CultureInfo.InvariantCulture));
                }
            }
            if (num2 != 0)
            {
