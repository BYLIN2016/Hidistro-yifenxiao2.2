namespace Hidistro.UI.Web.Admin.distribution
{
    ...
    public class EtaoReuest : AdminPage
    {
        ...
        private void BindRequests()
        {
            ...
            int total = 0;
            DataTable table = DistributorHelper.GetEtaoRequests(pagination, this.userName, this.trueName, out total);
        ...
        protected void grdDistributorSites_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                if (DistributorHelper.OpenEtao(Convert.ToInt32(e.CommandArgument)))
                {
                    this.ReBind(true);
                    this.ShowMsg("已通过审核！", true);
                }
            }
            else if ((e.CommandName == "refuse") && DistributorHelper.RefuseEtao(Convert.ToInt32(e.CommandArgument)))
            {
                this.ReBind(true);
