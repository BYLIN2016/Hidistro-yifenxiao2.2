namespace Hidistro.SaleSystem.DistributionData
{
    ...
    public class ShoppingData : ShoppingSubsiteProvider
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("update distro_CouponItems  set userName=@UserName,Userid=@Userid,Orderid=@Orderid,CouponStatus=@CouponStatus,EmailAddress=@EmailAddress,UsedTime=@UsedTime WHERE ClaimCode=@ClaimCode  and CouponStatus!=1");
        ...
        private bool AddOrderGiftItems(string orderId, IList<OrderGiftInfo> orderGifts, DbTransaction dbTran)
        {
            ...
            foreach (OrderGiftInfo info in orderGifts)
            {
                ...
                builder.Append("INSERT INTO distro_OrderGifts(OrderId, GiftId, GiftName, CostPrice, ThumbnailsUrl, Quantity,DistributorUserId,PromoType) VALUES( @OrderId,").Append("@GiftId").Append(str).Append(",@GiftName").Append(str).Append(",@CostPrice").Append(str).Append(",@ThumbnailsUrl").Append(str).Append(",@Quantity").Append(str).Append(",@DistributorUserId").Append(str).Append(",@PromoType").Append(str).Append(");");
        ...
        private bool AddOrderLineItems(string orderId, ICollection lineItems, DbTransaction dbTran)
        {
            ...
            {
                string str = num.ToString(CultureInfo.InvariantCulture);
                builder.Append("INSERT INTO distro_OrderItems(OrderId, DistributorUserId, SkuId, ProductId, SKU, Quantity, ShipmentQuantity, CostPrice, ").Append("ItemListPrice, ItemAdjustedPrice, ItemDescription, SKUContent, ThumbnailsUrl, Weight,PromotionId,PromotionName) VALUES(@OrderId,").Append("@DistributorUserId").Append(",@SkuId").Append(str).Append(",@ProductId").Append(str).Append(",@SKU").Append(str).Append(",@Quantity").Append(str).Append(",@ShipmentQuantity").Append(str).Append(",@CostPrice").Append(str).Append(",@ItemListPrice").Append(str).Append(",@ItemAdjustedPrice").Append(str).Append(",@ItemDescription").Append(str).Append(",@SKUContent").Append(str).Append(",@ThumbnailsUrl").Append(str).Append(",@Weight").Append(str).Append(",@PromotionId").Append(str).Append(",@PromotionName").Append(str).Append(");");
        ...
        public override CouponInfo GetCoupon(string couponCode)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT c.* FROM distro_Coupons c INNER  JOIN distro_CouponItems ci ON ci.CouponId = c.CouponId Where ci.ClaimCode = @ClaimCode   and  CouponStatus=0 and @DateTime>c.StartTime  AND @DateTime < c.ClosingTime AND DistributorUserId = @DistributorUserId;");
        ...
        {
            int num = 0;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT ISNULL(SUM(Quantity),0) as Quantity FROM distro_GiftShopingCarts WHERE UserId = @UserId AND PromoType=@PromoType");
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, HiContext.Current.User.UserId);
            this.database.AddInParameter(sqlStringCommand, "PromoType", DbType.Int32, (int) promotype);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        public override OrderInfo GetOrderInfo(string orderId)
        {
            ...
            query = query + "SELECT * FROM distro_OrderGifts WHERE OrderId = @OrderId;" + "SELECT o.*,(SELECT Stock FROM Hishop_SKUs WHERE SkuId=o.SkuId) as Stock FROM distro_OrderItems o WHERE o.OrderId = @OrderId;";
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DELETE FROM distro_GiftShopingCarts WHERE UserId = @UserId AND GiftId = @GiftId AND PromoType=@PromoType");
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, HiContext.Current.User.UserId);
            this.database.AddInParameter(sqlStringCommand, "GiftId", DbType.Int32, giftId);
            this.database.AddInParameter(sqlStringCommand, "PromoType", DbType.Int32, (int) promotype);
            this.database.ExecuteNonQuery(sqlStringCommand);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE distro_GiftShopingCarts SET Quantity = @Quantity WHERE UserId = @UserId AND GiftId = @GiftId AND PromoType=@PromoType");
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, HiContext.Current.User.UserId);
            this.database.AddInParameter(sqlStringCommand, "GiftId", DbType.Int32, giftId);
            this.database.AddInParameter(sqlStringCommand, "Quantity", DbType.Int32, quantity);
            this.database.AddInParameter(sqlStringCommand, "PromoType", DbType.Int32, (int) promotype);
